//
//  FraudsterToast.swift

import UIKit

final public class FraudsterToast {
...
final fileprivate class LoafManager: LoafDelegate {
    ...
    fileprivate var queue = Queue<FraudsterToast>()
    fileprivate var isPresenting = false
    
    fileprivate func queueAndPresent(_ loaf: FraudsterToast) {
        queue.enqueue(loaf)
        presentIfPossible()
    }
...
final class LoafViewController: UIViewController {
    ...
    init(_ toast: FraudsterToast) {
        self.loaf = toast
        self.transDelegate = Manager(loaf: toast, size: .zero)
        super.init(nibName: nil, bundle: nil)
		
        var width: CGFloat?
        if case let FraudsterToast.State.custom(style) = loaf.state {
    ...
    override func viewDidLoad() {
        ...
        switch loaf.state {
        case .success:
            imageView.image = FraudsterToast.Icon.success
            view.backgroundColor = UIColor(hexString: "#2ecc71")
            constrainWithIconAlignment(.left)
        case .warning:
            imageView.image = FraudsterToast.Icon.warning
            view.backgroundColor = UIColor(hexString: "##f1c40f")
            constrainWithIconAlignment(.left)
        case .error:
            imageView.image = FraudsterToast.Icon.error
            view.backgroundColor = UIColor(hexString: "##e74c3c")
            constrainWithIconAlignment(.left)
        case .info:
            imageView.image = FraudsterToast.Icon.info
    ...
    private func constrainWithIconAlignment(_ alignment: FraudsterToast.Style.IconAlignment, showsIcon: Bool = true) {
