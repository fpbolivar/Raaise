...
final class Controller: UIPresentationController {
    private let loaf: FraudsterToast
    ...
    override func presentationTransitionWillBegin() {
        ...
        if #available(iOS 11, *) {
            ...
            let statusBarSize =  UIScreen.main.bounds.size //UIWindow.keyWin?.windowScene?.statusBarManager?.statusBarFrame.size
            //let statusBarSize = UIApplication.shared.statusBarFrame.size
            containerInsets = UIEdgeInsets(top: min(statusBarSize.width, statusBarSize.height ?? 60), left: 0, bottom: 0, right: 0)
        }
