...
//
import UIKit
protocol ShareVideoDelegate{
    //share video
    func shareVideo(data: ShareVideoModel)
}
class SharePopUp: BaseControllerVC {
   ...
   // @IBOutlet weak var otherImg:UIImageView!
    ...
    var videoLink :  String = "" //get the link of the video to be shared
    var userListData:[ChatChannelModel] = []
    var delegate:SharePopUpDelegate?
    var videoId = ""
    var receiverId = ""
    var page = 1
    var vm = SharePopUpVM() //view model
    override func viewDidLoad() {
        super.viewDidLoad()
        hideNavbar()
        vm.configureUI(VC:self)
...
//MARK: EXTENSION OF SHAREVIDEO DELEGATE
extension SharePopUp : ShareVideoDelegate{
    ///***Function s

    //MARK: SHARE VIDEO
    func shareVideo(data: ShareVideoModel) {
        print("\(#function) \(data.shareOn)")
        switch data.shareOn {
        case .other:
            //share video by opening sheet
            GlobalManager.shareVideoByOpeningSheet(link: videoLink)
        default:
            break
        }
    }
}
//MARK: EXTENSION OF COLLECTION VIEW DELEGATE
extension SharePopUp: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ///***Functions
    
    //MARK: NUMBER OF ITEMS
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return vm.shareModel.count
    }
    //MARK: CELL FOR ITEM
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        return SharePopUpCellSetup.useShareVideoCell(collectionView: collectionView, indexPath: indexPath, delegate: self, data: vm.shareModel[indexPath.row])
    }
    //MARK: SIZE FOR ITEM
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let width = collectionView.frame.width / CGFloat(vm.shareModel.count)
        let height = Constant.ShareVideo.SHARE_COLLECTION_HEIGHT
