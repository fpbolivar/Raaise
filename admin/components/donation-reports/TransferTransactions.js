 ...
 csvExport = async () => {
  document.getElementById("custom-loader").style.display = "block";
    try {
      ...
      if (data.status == 200) {
        ...
        const file = "transfertransactions.csv";
        const arr = [];
        const col = ["Username", "Email Address ", "Phone Number", "Transaction Id","Status","Amount"];
  ...
  getData = async (obj) => {
    try {
      ...
      const { data } = await axios.post(`${DONATION.transferredTransaction}`, {  //API calling
        page: pageNumber,
        limit: dataLimit,
        startDate:startDate,
        endDate:endDate,
  ...
  render() {
    const {columns,transferData,pagination} = this.state;
    return (
      <Container>
        <Wrapper>
          <Header>
          ...
          <FilteringTable data={transferData} columns={columns} pagination={pagination} handleTable={this.getData} setDateFilter={this.setDateFilter} isDateFilter={true}/>
        </Wrapper>
      </Container>
    );
  }
